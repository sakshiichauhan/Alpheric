...
const SolutionCard: React.FC<SolutionCardProps> = ({
  ...
  // 👇 Only special-case the LAST card
  const isLast = index === totalCards - 1;
  const isFirst = index === 0;
  const isSecond = index === 1;

  // IMAGE: last card holds (no fade-out). Others unchanged.
  const imageOpacity = useTransform(
    smoothScrollYProgress,
    [fadeInStart, fadeInEnd, fadeOutStart, fadeOutEnd],
    isLast ? [0, 1, 1, 1] : isFirst ? [1, 1, 1, 1] : isSecond ? [1, 1, 1, 1] : [0, 1, 1, 0]
  );
  ...
  const textY = useTransform(
    ...
    isFirst ? [0, 0, 0, 0] : [300, 0, 0, -550]
  );

  const textOpacity = useTransform(
    smoothScrollYProgress,
    [fadeInStart, fadeInEnd, fadeOutStart, fadeOutEnd],
    isFirst ? [1, 1, 1, 1] : [0, 1, 1, 0]
  );

  // Keep your existing transition (unchanged)
